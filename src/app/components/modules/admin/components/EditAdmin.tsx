...
const EditAdmin = ({ isOpen, onClose, user, onSuccess }: any) => {
    ...
    // Access control layout configuration flag definitions
    const isFieldDisabled = isAdmin;
    const isUploadDisabled = isAdmin;
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await adminService.updateAdmin(user.id, data);

            if (formData.role !== user.role && adminService.hasRoleUpdateMethod()) {
                await adminService.updateAdminRole(user.id, formData.role);
            }
            ...
            <div className="w-full max-w-[650px] card-theme rounded-[30px] md:rounded-[45px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[80dvh]">
                ...
                <div className="flex items-center justify-between p-6 md:p-8 pb-4 shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="p-3 custom-main-color-card rounded-2xl text-white">
                            <PencilLine className="custom-main-color-icon w-5 h-5 md:w-6 md:h-6" strokeWidth={2.5} />
                        </div>
                        <h2 className="text-lg md:text-xl font-black text-[var(--header-text)]">Edit Admin</h2>
                    </div>
                ...
                <div className="flex-1 p-6 md:p-8 py-0 space-y-6 overflow-y-auto no-scrollbar">
                    <div className="flex justify-center py-2">
                        <div className={`relative group ${isUploadDisabled ? 'cursor-not-allowed' : ''}`}>
                            <div
                            ...
                            {selectedFile ? (
                                <div className="absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 flex gap-1.5">
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            if (!isUploadDisabled) setShowCropper(true);
                                        }}
                                        disabled={isUploadDisabled}
                                        className={`p-2 rounded-xl shadow-lg border-2 border-white transition-all ${
                                            isUploadDisabled
                                                ? 'bg-slate-300 opacity-50 pointer-events-none'
                                                : 'bg-[#8ABEB9] text-white hover:bg-[#7aa9a4] cursor-pointer'
                                        }`}
                                    >
                                        <Edit2 size={14} />
                                    </button>
                                ...
                                <div
                                    className={`absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 p-2 text-white rounded-xl shadow-lg border-4 border-white transition-all ${
                                        isUploadDisabled
                                            ? 'bg-slate-300 opacity-50 pointer-events-none'
                                            : 'custom-main-color-bg group-hover:scale-110 cursor-pointer'
                                    }`}
                                    onClick={() => !isUploadDisabled && fileInputRef.current?.click()}
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                        <div className="space-y-1 md:space-y-2">
                            ...
                            <input
                                ...
                                className={`w-full input-theme rounded-[15px] md:rounded-[20px] py-3.5 md:py-4 px-4 text-[13px] md:text-[14px] font-bold transition-all outline-none ${
                                    isFieldDisabled ? 'bg-slate-50 text-slate-400 opacity-70 cursor-not-allowed' : 'text-slate-600 focus:bg-white focus:border-blue-400'
                                }`}
                            />
                        </div>
                        <div className="space-y-1 md:space-y-2">
                        ...
                        <div className="md:col-span-2 space-y-1 md:space-y-2">
                            ...
                            <div className="relative">
                                ...
                                >
                                    {/* Fix: Hide super-admin option if current user is only an admin */}
