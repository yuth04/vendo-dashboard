...
const EditCoupons = ({ coupon, onClose, onSuccess }: EditCouponProps) => {
    ...
    const [previewUrl, setPreviewUrl] = useState<string>(getImageUrl((coupon as any).image));
    ...
    const [formData, setFormData] = useState({
        ...
        discount_id: c.discount_id || c.discount_category_id || ""
    });
    ...
    useEffect(() => {
        ...
        setPreviewUrl(getImageUrl(c.image));
        setIsNewImage(false);
        setImageFile(null);
    }, [coupon]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            // 2MB size rule validation checks
    ...
    // Smart rollbacks on click action handler
    const handleRemoveFile = (e: React.MouseEvent) => {
        ...
        if (isNewImage) {
            // Revert back to database original image setting
            setImageFile(null);
            setPreviewUrl(getImageUrl((coupon as any).image));
            setIsNewImage(false);
        } else {
            // Remove the active old image completely from layout
            setImageFile(null);
            setPreviewUrl("");
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await couponService.updateCoupon(coupon.id, payload);
            if (!response.error) {
                showToast("Coupon updated successfully!", "success");
                onSuccess();
            } else {
                showToast(response.error.message || "Failed to update", "error");
            }
        } catch (error) {
            ...
            <div className="w-[95%] sm:w-full max-w-[600px] card-theme rounded-[24px] md:rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[80dvh]">
                <div className="flex items-center justify-between px-6 py-5 border-b border-gray-50">
                    <div className="flex items-center gap-2">
                        <Edit3 className="custom-main-color-icon w-5 h-5"/>
                        <h2 className="text-[17px] font-black text-[var(--header-text)]">Edit Coupon</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 no-scrollbar">
                    ...
                    {/* Image Header with Max 2MB notice */}
                            ...
                            >
                                {previewUrl ? (
                                    <>
                                        ...
                                        {/* Custom interactive close and undo action button component */}
                                        <button
                                            type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <InputLabel text="Promotion Name"/>
