...
import EditCustomers from "@/src/app/components/modules/customers/components/EditCustomers";
import { UserListResponse } from "@/src/app/components/modules/customers/core/models/customerModel";
...
const DetailsCustomers = () => {
    ...
    const fetchCustomer = useCallback(
        (signal?: AbortSignal) => customerService.getCustomerById(userId, signal),
        [userId]
    );

    const { data, loading, error, refetchData } = useCustomerData<UserListResponse>(
        fetchCustomer,
        undefined,
        !!userId
    );
    ...
    const handleToggleStatus = async () => {
        if (!canChangeStatus) {
            showToast("Only super-admin can activate or deactivate customer status.", "error");
            return;
        }
        ...
        if (confirmed) {
            try {
                await customerService.updateStatus(customer!.id, newStatus, customer as any);
    ...
    const handleDelete = async () => {
        if (!canDelete) {
            showToast("Only super-admin can delete customer records.", "error");
            return;
        }
    ...
    if (error || !customer) {
        return (
            <DataNotFound
                ...
                message="The customers ID requested does not exist or has been removed."
                icon={Users}
            />
        );
    }
    ...
    return (
        <div className="min-h-screen sm:p-4 py-4 md:p-8">
            ...
            <nav className="flex items-center gap-2 text-[12px] sm:text-[14px] font-black text-gray-400 uppercase tracking-[0.2em] mb-6 overflow-x-auto whitespace-nowrap pb-2 no-scrollbar">
                <span className="custom-main-color-text-hover cursor-pointer transition-colors" onClick={() => router.push('/admin/dashboard')}>Dashboard</span>
                <ChevronRight size={12} className="text-gray-300" />
                <span className="custom-main-color-text-hover cursor-pointer transition-colors" onClick={() => router.push('/admin/customers')}>Customers</span>
                <ChevronRight size={12} className="text-gray-300" />
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-[24px] sm:text-[30px] font-black text-[var(--header-text)] tracking-tight">Customer Profile</h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div
                        onClick={!canChangeStatus ? handleToggleStatus : undefined}
                        className={`relative ${!canChangeStatus ? 'cursor-not-allowed' : ''}`}
                    >
                        <button
                            onClick={canChangeStatus ? handleToggleStatus : undefined}
                            className={`flex items-center gap-2 px-3 py-2 sm:px-4 sm:py-3 bg-white border rounded-2xl text-[12px] sm:text-[14px] font-black shadow-sm transition-all ${
                                !canChangeStatus
                                    ? 'border-emerald-100 text-emerald-600 opacity-50 cursor-not-allowed pointer-events-none'
                    ...
                    <div
                        ...
                        className={`relative ${!canDelete ? 'cursor-not-allowed' : ''}`}
                    >
                        <button
                            onClick={canDelete ? handleDelete : undefined}
                            className={`px-3 py-2 sm:px-4 sm:py-3 bg-white border rounded-2xl shadow-sm transition-all ${
                                !canDelete
                                    ? 'border-red-50 text-red-500 opacity-50 cursor-not-allowed pointer-events-none'
            ...
            {/* The rest of your profile UI layout stays exactly identical */}
                ...
                <div className="xl:col-span-2 space-y-6">
                    <div className="card-theme border border-gray-100 rounded-[40px] p-8 md:p-12 shadow-sm relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            <div className="flex items-center gap-4 p-5 card-theme rounded-[24px] border border-gray-100 group">
                            ...
                            <div className="flex items-center gap-4 p-5 card-theme rounded-[24px] border border-gray-100 transition-all group">
                            ...
                            <div className="flex items-center gap-4 p-5 card-theme rounded-[24px] border border-gray-100 transition-all group">
                                ...
                                <div>
                                    ...
                                    <p className="text-[12px] font-bold text-gray-500">{customer.created_by?.created_at || "N/A"}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="xl:col-span-1 space-y-6">
                    <div className="card-theme rounded-[32px] p-8 shadow-sm">
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="text-[14px] font-black ttext-[var(--header-text)] block mb-2">Internal ID</label>
                            ...
                            <div className="pt-6 border-t border-gray-50 space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-[12px] font-black text-[var(--header-text)]">{customer.created_by?.created_at || "N/A"}</span>
                                </div>
                                <div className="flex justify-between items-center">
