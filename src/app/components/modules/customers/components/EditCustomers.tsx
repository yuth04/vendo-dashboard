...
const EditCustomers = ({ isOpen, onClose, user, onSuccess }: any) => {
    ...
    const createCroppedImage = async (imageSrc: string, pixelCrop: any): Promise<{file: File, url: string} | null> => {
        ...
        return new Promise((resolve) => {
            canvas.toBlob((blob) => {
                ...
                const file = new File([blob], "customer-avatar.jpg", { type: "image/jpeg" });
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            // Execute metadata details patch update if allowed
            await customerService.updateCustomer(user.id, data);

            if (formData.role !== user.role) {
                await customerService.updateCustomerRole(user.id, formData.role);
            }
            ...
            <div className="w-full max-w-[650px] card-theme rounded-[30px] md:rounded-[45px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[80dvh]">
                ...
                <div className="flex items-center justify-between p-6 md:p-8 pb-4 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-lg md:text-xl font-black text-[var(--header-text)]">Edit Customer</h2>
                    </div>
                ...
                <div className="flex-1 p-6 md:p-8 py-0 space-y-6 overflow-y-auto no-scrollbar">
                        ...
                        <div className="relative group">
                            <div
                                className={`w-28 h-28 md:w-32 md:h-32 rounded-[30px] md:rounded-[35px] bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex items-center justify-center transition-all ${
                                    canUploadImage ? 'group-hover:border-blue-400 cursor-pointer' : 'opacity-65 cursor-not-allowed'
                                }`}
                            ...
                            {/* Image Actions Overlay Guarded by Permissions */}
                            {selectedFile ? (
                                ...
                                <div
                                    className={`absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 p-2 text-white rounded-xl shadow-lg border-4 border-white transition-all ${
                                        canUploadImage ? 'custom-main-color-bg group-hover:scale-110 cursor-pointer' : 'bg-gray-300 opacity-50 cursor-not-allowed'
                                    }`}
                                    onClick={() => canUploadImage && fileInputRef.current?.click()}
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                        ...
                        {/* User Role dropdown selector configuration */}
                        <div className="md:col-span-2 space-y-1 md:space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none w-4.5 h-4.5" />
                            </div>
                        </div>
                    </div>
                </div>
