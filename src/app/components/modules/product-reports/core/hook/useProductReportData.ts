...
interface ProductReportApiError {
    message: string;
    code?: string | number;
}

export function useProductReportData<T>(
    fetchFn: (signal?: AbortSignal) => Promise<ApiResponse<T>>,
    defaultValue?: T,
    enableClientFetch: boolean = false,
) {
    const [data, setData] = useState<T | undefined>(defaultValue);
    const [error, setError] = useState<ProductReportApiError| null>(null);
    ...
    useEffect(() => {
        ...
        fetchFn(controller.signal)
            .then((res) => {
                if (res.error) {
                    setError(res.error as ProductReportApiError);
