...
import { Plus, X, Upload, ChevronDown, Loader2 } from "lucide-react";
import { useAlert } from "@/src/app/components/context/AlertContext";
import { categoriesClient } from "@/src/app/components/modules/settings/categories/core/api/categoriesClient";
import { CategoryService } from "../core/services/categoriesService";
import { ParentCategory } from "@/src/app/components/modules/settings/categories/core/models/categoriesModel";
...
const AddCategoryModal = ({ isOpen, onClose, onRefresh }: AddCategoryModalProps) => {
    ...
    const handleSubmit = async () => {
        ...
        if (name.trim().length > 255) {
            return showToast("Category name cannot exceed 255 characters", "warning");
        }
        if (slug.length > 255) {
            return showToast("Generated slug is too long (exceeds 255 characters)", "warning");
        }

        setLoading(true);
        try {
            const res = await CategoryService.createCategoryLogic({
                name: name.trim(),
                slug,
                status,
                parentId,
            ...
            <div className="w-full max-w-[500px] rounded-[32px] md:rounded-[40px] card-theme shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[80dvh]">
                ...
                <div className="flex items-center justify-between border-b border-gray-50 p-6 md:p-8 shrink-0">
                    <div className="flex items-center gap-3 text-gray-900 font-black text-lg md:text-xl">
                        ...
                        <span className="text-[var(--header-text)]">New Category</span>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-8 space-y-6">
                    <div className="flex justify-center">
                        <div className=" relative flex h-28 w-28 md:h-32 md:w-32 cursor-pointer flex-col items-center justify-center rounded-[28px] md:rounded-[32px] border-2 border-dashed border-gray-200 bg-gray-50/50 transition-all custom-main-color-border-hover overflow-hidden">
                            {previewUrl ? (
                                <img src={previewUrl} className="h-full w-full object-cover" alt="preview" />
                            ) : (
                                <>
                                    <Upload className="mb-1 text-gray-400 custom-main-color-text-hover" size={24} />
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="py-1">
                                <label className="text-[12px] sm:text-[14px] font-medium text-gray-500">Category
                                    Name</label>
                            </div>
                            <input
                                type="text"
                        ...
                        <div>
                            <div className="py-1">
                                <label
                                    className="text-[12px] sm:text-[14px] font-medium text-gray-500">Slug</label>
                            </div>
                            <input
                                type="text"
                                value={slug}
                    ...
                    <div>
                        <div className="py-1">
                            <label
                                className="text-[12px] sm:text-[14px] font-medium text-gray-500">Status</label>
                        </div>
                        <div className="flex h-[50px] md:h-[58px] w-full items-center rounded-[20px] card-theme p-1.5">
                            <button
                                type="button"
                                onClick={() => setStatus('ACTIVE')}
                                className={`flex-1 h-full rounded-[20px] text-[10px] md:text-[12px] font-black transition-all cursor-pointer ${status === 'ACTIVE' ? 'bg-emerald-50 border border-emerald-200 text-emerald-600' : 'text-gray-500'}`}
                            >
                                Active
                            </button>
                            <button
                                ...
                                className={`flex-1 h-full rounded-[20px] text-[10px] md:text-[12px] font-black transition-all cursor-pointer ${status === 'INACTIVE' ? 'bg-white text-gray-500 shadow-sm border border-gray-100' : 'text-gray-500'}`}
                            >
                                Inactive
                            </button>
                        </div>
