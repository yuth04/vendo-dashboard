...
import {X, Upload, Share2, Image as ImageIcon, Trash2, Link as LinkIcon, Edit3} from "lucide-react";
...
const EditSliderModal = ({ isOpen, onClose, refreshData, editData }: EditSliderModalProps) => {
        ...
        if (isOpen) {
            if (editData) {
                setFormData({
                    ...
                    status: editData.status === 1 || editData.status === true || editData.status === "active"
                });
            } else {
                setFormData({
                    title: "",
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.imagePreview) return showToast("Image is required", "warning");

        setLoading(true);
        try {
            const res = isEdit
                ? await SliderService.updateSliderLogic(editData.id, formData)
                : await SliderService.createSliderLogic(formData);

            if (!res.error) {
                showToast(isEdit ? "Slider updated successfully" : "Slider created successfully", "success");
                refreshData();
                onClose();
            } else {
            ...
            <div className="w-full max-w-[600px] rounded-[30px] md:rounded-[40px] card-theme shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[80dvh] overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-gray-50 p-6 md:p-8 shrink-0">
                    <div className="flex items-center gap-2">
                        <div className="p-2 custom-main-color-card rounded-xl">
                            <Edit3 className="custom-main-color-icon w-4 h-4 md:w-5 md:h-5" strokeWidth={3}/>
                        </div>
                        <h2 className="text-[18px] md:text-[22px] font-black text-[var(--header-text)]">{isEdit ? 'Edit Slider' : 'New Slider'}</h2>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Summer Sale – Up to 80% Off!"
                            />
                        </div>
